...
from model.VFIT.Sep_STS_Encoder import ResBlock
...
class UNet_3D_3D(nn.Module):
    def __init__(self, n_inputs=4, joinType="concat", ks=5, dilation=1):
        ...
        #
        from model.VFIT.Sep_STS_Encoder import SepSTSEncoder
        ...
        def Subnet_Dmap():
            return torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=2048, out_channels=1024, kernel_size=3, stride=1, padding=1),
    ...
    def forward(self, frames):
        ...
        if h0 % 32 != 0:
            pad_h = 32 - (h0 % 32)
            frames[0] = F.pad(frames[0], (0, 0, 0, pad_h), mode='reflect')
            frames[1] = F.pad(frames[1], (0, 0, 0, pad_h), mode='reflect')
            frames[2] = F.pad(frames[2], (0, 0, 0, pad_h), mode='reflect')
            frames[3] = F.pad(frames[3], (0, 0, 0, pad_h), mode='reflect')
            h_padded = True

        if w0 % 32 != 0:
            pad_w = 32 - (w0 % 32)
            frames[0] = F.pad(frames[0], (0, pad_w, 0, 0), mode='reflect')
            frames[1] = F.pad(frames[1], (0, pad_w, 0, 0), mode='reflect')
            frames[2] = F.pad(frames[2], (0, pad_w, 0, 0), mode='reflect')
            frames[3] = F.pad(frames[3], (0, pad_w, 0, 0), mode='reflect')
            w_padded = True
        ...
        b, c, t, h, w = x_4.size()
        D_input = x_4.view(b, -1, h, w)
        Dmap = self.moduleD(D_input)

        out = frames[1] * Dmap + out * (1 - Dmap)
...
class SynBlock(nn.Module):
    def __init__(self, n_inputs, nf, ks, dilation, norm_weight=True):
        ...
        import model.VFIT.cupy_module.adacof as adacof
